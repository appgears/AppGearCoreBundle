...
class ModelGenerator
{
    ...
    public function generate(Model $model)
    {
        ...
        if (in_array($name, $this->alreadyGeneratedModels)) {
            return;
        }

        // Фиксируем модель как сгенерированную
        $this->alreadyGeneratedModels[] = $name;

        $className = $this->modelManager->className($name);
        ...
        // Сохраняем код модели в файл
        $this->save($name, (string) $file);
    }
     ...
     * @param string $source Исходный код модели
     */
    private function save($name, $source)
    {
        $path    = $this->getModelPath($name) . '.new.php';
